...
def get_vendor_info(path):
    ...
    additional_rows = pd.DataFrame(
        {"SUBJECT_CODE": range(161, 201), "VENDOR": [None] * 40}
    )

    vendor_info = pd.concat([vendor_info, additional_rows], ignore_index=True)
...
def get_splits_supervised(vendor_info, dict_path):
    vendor_list = []
...
def preprocess_image(id, image, crop, is_seg, spacing, spacing_target):
    ...
    if not is_seg:
        image = np.stack(
            [
                resize(
                    slice, new_shape[1:], 3, cval=0, mode="edge", anti_aliasing=False
                )
                for slice in image
            ]
        )
...
def preprocess(patient_info, spacing_target, folder_out, soft_preprocessing=False):
    if not os.path.isdir(os.path.join(folder_out, "SA")):
        os.makedirs(os.path.join(folder_out, "SA"))
    if not os.path.isdir(os.path.join(folder_out, "LA")):
        os.makedirs(os.path.join(folder_out, "LA"))
            ...
            for j in ["ED", "ES"]:
                ...
                if soft_preprocessing and i == "_gt":
                    ...
                    image = preprocess_image(
                        ...
                        patient_info[id]["crop_{}".format(j)],
                        i == "_gt",
...
class RandomCropTransform:
    ...
    def __call__(self, sample):
        ...
        sample["data"], sample["gt"] = random_crop(
            sample["data"], sample["gt"], self.crop_size
        )
...
class SpatialTransform:
    ...
    def __call__(self, sample):
        sample["data"] = sample["data"][None, None, :, :]
        sample["gt"] = sample["gt"][None, None, :, :]
        sample["data"], sample["gt"] = augment_spatial(
            sample["data"], sample["gt"], **self.params
        )
        return {"data": sample["data"][0, 0], "gt": sample["gt"][0, 0]}
...
class AddPadding:
    ...
    def __call__(self, sample):
        ...
        sample = {
            k: self.resize_image_by_padding(v, new_shape=self.output_size)
            if isinstance(v, np.ndarray) else v  # Apply padding only if it's a numpy array
...
class CenterCrop:
    ...
    def __call__(self, sample):
        sample = {
            k: self.center_crop_2D_image(v, center_crop=self.output_size)
...
class RandomCrop:
    ...
    def __call__(self, sample):
        sample = {
            k: self.random_crop_2D_image(v, crop_size=self.output_size)
            for k, v in sample.items()
        }
        return sample
            ...
            if isinstance(sample["gt"], list):
                sample["gt"] = [
                    (
                        ...
                        if len(y.shape) == 3
                        else torch.from_numpy(y).float()[None, :, :]
                    )
                    for y in sample["gt"]
                ]
...
class Downsample:
    def downsample_seg_for_ds_transform2(
        self,
        seg,
        ds_scales=[[1, 1, 1]] + [2 * [1 / (2**n)] for n in range(1, 6)],
        order=0,
        cval=0,
        axes=None,
    ):
...
transform = torchvision.transforms.Compose(
    [AddPadding((256, 256)), CenterCrop((256, 256)), OneHot(), ToTensor()]
)
...
transform_downsample = torchvision.transforms.Compose(
    [AddPadding((256, 256)), CenterCrop((256, 256)), Downsample(), OneHot(), ToTensor()]
)
...
class ACDCAllPatients(torch.utils.data.Dataset):
    def __init__(self, root_dir, transform=None, transform_gt=True):
        self.dataloader = ACDCDataLoader(
...
class Validator:
    ...
    def get_pseudo_summary(self, domain_id, phases=["ED", "ES"]):
        return {
            "p" + k: v
            for k, v in self._get_summary(
                self.pseudo_metrics[domain_id], phases
            ).items()
        }

    def process_labPatient(self, model, loader, dann=False):
        prediction, gt = [], []
        vendor_preds, vendor_gt = [], []
        for batch in loader:
            batch["data"] = batch["data"].to(device)
            if dann:
                batch["output"], batch["vendor_pred"] = model(batch["data"])
            else:
            ...
            prediction = (
                torch.cat([prediction, batch["output"]], dim=0)
                if len(prediction) > 0
                else batch["output"]
            )
            ...
            if dann:
                if isinstance(batch["vendor_pred"], list):
                    batch["vendor_pred"] = batch["vendor_pred"][0]
                batch["vendor"] = (torch.stack((batch["vendor"][0], batch["vendor"][1], batch["vendor"][2]), dim=1)).to(device) # Move vendor ground truth to device
                vendor_preds = (
                    torch.cat([vendor_preds, batch["vendor_pred"]], dim=0)
                    if len(vendor_preds) > 0
                    else batch["vendor_pred"]
                )
                vendor_gt = torch.cat([vendor_gt, batch["vendor"]], dim=0) if len(vendor_gt) > 0 else batch["vendor"]
        ...
        if dann:
            vendor_preds = vendor_preds.cpu().numpy()
            vendor_gt = vendor_gt.cpu().numpy()
            return [prediction, vendor_preds], [gt, vendor_gt]
    ...
    def process_ulabPatient(self, model, loader, reconstructor):
        ...
        for batch in loader:
            ...
            batch["pgt"] = (
                nn.functional.one_hot(torch.argmax(batch["pgt"], dim=1), num_classes=4)
                .permute(0, 3, 1, 2)
                .float()
            )
            pgt = (
                torch.cat([pgt, batch["pgt"]], dim=0) if len(pgt) > 0 else batch["pgt"]
            )
        prediction, pgt = (
            {
                "ED": segmentation.cpu().numpy()[: len(segmentation) // 2],
    ...
    def evaluate_metrics(self, model, prediction, target, dann):
        epoch_metrics = {}
        if dann:
            for k, v in {
                **model.Loss(
                    torch.tensor(prediction[0]).to(device),
                    torch.tensor(target[0]).to(device),
                    torch.tensor(prediction[1]).to(device),
                    torch.tensor(target[1]).to(device), 
                    validation=True
                ),
                **model.Metrics(
                    np.argmax(prediction[0], axis=1), 
                    np.argmax(target[0], axis=1), 
                    np.argmax(prediction[1], axis=1), 
                    np.argmax(target[1], axis=1)
                ),
            }.items():
                epoch_metrics[k] = v
        else:
            for k, v in {
                **model.Loss(torch.tensor(prediction).to(device), torch.tensor(target).to(device), validation=True),
                **model.Metrics(np.argmax(prediction, axis=1), np.argmax(target, axis=1)),
            }.items():
                epoch_metrics[k] = v
        return epoch_metrics

    def patient_evaluation(self, patient_id, model, prediction, target, metrics, dann=False):
        for phase in ["ED", "ES"]:
            epoch_metrics = {}
            if dann:
                epoch_metrics = self.evaluate_metrics(model, [prediction[0][phase], prediction[1]], [target[0][phase], target[1]], dann)
            else:
                epoch_metrics = self.evaluate_metrics(model, prediction[phase], target[phase], dann)
            ...
            for k, v in epoch_metrics.items():
                metrics[patient_id][phase][k].append(v)
    ...
    def update_best_models(self, domain_id, model, checkpoint):
        ...
        if value < np.max(self.best_models[domain_id]):
            ...
            self.best_models[domain_id] = sorted(self.best_models[domain_id])[
                : self.m_best
            ]
            index = self.best_models[domain_id].index(value)
            for model_id in range(index + 1, self.m_best)[::-1]:
                checkpoint.rename(
                    "best_{:03d}".format(model_id - 1), "best_{:03d}".format(model_id)
                )
    ...
    def update_best_predictions(
        self, domain_id, patient_id, prediction, pseudo_metrics
    ):
        ...
        for phase in ["ED", "ES"]:
            ...
            if pseudo_dc == np.max(pseudo_metrics[phase]["RV_dc"]):
                ...
                if pseudo_hd < self.get_thrs(domain_id, phase, "RV_hd")[1]:
                    self.best_predictions[domain_id][patient_id][phase]["ensemble"] = (
                        prediction[phase]
                    )

    def domain_evaluation(
        self, domain_id, model, loader, checkpoint=None, reconstructor=None, dann=False
    ):
        for patient in loader:
            if reconstructor is None:
                if domain_id not in self.metrics:
                    self.metrics[domain_id] = {}
                prediction, gt = self.process_labPatient(model, patient, dann)
                self.patient_evaluation(patient.dataset.id, model, prediction, gt, self.metrics[domain_id], dann)
            else:
                if domain_id not in self.pseudo_metrics:
                    self.pseudo_metrics[domain_id] = {}
                prediction, pgt = self.process_ulabPatient(
                    model, patient, reconstructor
                )
                self.patient_evaluation(
                    patient.dataset.id,
                    model,
                    prediction,
                    pgt,
                ...
                self.update_best_predictions(
                    domain_id,
                    patient.dataset.id,
                    prediction,
                    self.pseudo_metrics[domain_id][patient.dataset.id],
                )
        if reconstructor is None:
    ...
    def get_best_prediction(self, domain_id, patient_id):
        best_prediction = {}
        for phase in ["ED", "ES"]:
            best_prediction[phase] = self.best_predictions[domain_id][patient_id][
                phase
            ]["ensemble"]
    ...
    def get_anomalies(self, domain_id):
        ...
        for phase in ["ED", "ES"]:
            for patient_id in self.pseudo_metrics[domain_id]:
                pseudo_dc = np.max(
                    self.pseudo_metrics[domain_id][patient_id][phase]["RV_dc"]
                )
                if pseudo_dc < self.get_thrs(domain_id, phase, "RV_dc")[0]:
                    anomalies[phase].append(patient_id)
                    continue
                pseudo_hd = np.min(
                    self.pseudo_metrics[domain_id][patient_id][phase]["RV_hd"]
                )
                if pseudo_hd > self.get_thrs(domain_id, phase, "RV_hd")[1]:
                    anomalies[phase].append(patient_id)
        return anomalies
...
class Checkpointer:
    ...
    def rename(self, src, dst):
        if not src.endswith(".pth"):
            src += ".pth"
        if not dst.endswith(".pth"):
            dst += ".pth"
        if os.path.isfile(os.path.join(self.ckpt_folder, src)):
            os.rename(
                os.path.join(self.ckpt_folder, src), os.path.join(self.ckpt_folder, dst)
            )
            os.rename(
    ...
    def __call__(self, ckpt_name, model, validator):
        if not ckpt_name.endswith(".pth"):
            ckpt_name += ".pth"
        torch.save(
            {"M": model.state_dict(), "M_optim": model.optimizer.state_dict()},
            os.path.join(self.ckpt_folder, ckpt_name),
        )
        with open(
            os.path.join(self.ckpt_folder, ckpt_name.replace(".pth", "_val.pkl")), "wb"
        ) as f:
            pickle.dump(validator, f)

def dann_training(model, epochs, train_loader, val_loader, validator, checkpoint):
    model.eval()
    with torch.no_grad():
        validator.domain_evaluation("val", model, val_loader, checkpoint, dann=True)
    ...
    for epoch in epochs:
        ...
        for i, batch in enumerate(train_loader):
            ...
            batch["vendor"] = (torch.stack((batch["vendor"][0], batch["vendor"][1], batch["vendor"][2]), dim=1)).to(device)

            p = float(i + epoch * len(train_loader)) / (EPOCHS * len(train_loader))
            coeff = 2.0 / (1.0 + math.exp(-10.0 * p)) - 1.0

            seg_outputs, domain_outputs = model(batch["data"], alpha=coeff)
            loss = sum(
                [
                    w * model.Loss(x_seg, y_seg, x_class, y_class)
                    for w, x_seg, y_seg, x_class, y_class in zip(model.weights, seg_outputs, batch["gt"], domain_outputs, batch['vendor'])
                    if w != 0
                ]
            )
            loss.backward()
        ...
        if epoch % 10 == 0:
            checkpoint("{:03d}".format(epoch), model, validator)
    return
    ...
    for epoch in epochs:
        ...
        for batch in train_loader:
            ...
            loss = sum(
                [
                    w * model.Loss(x, y)
                    for w, x, y in zip(model.weights, batch["output"], batch["gt"])
                    if w != 0
                ]
            )
            loss.backward()
...
def plot_history(history, name):
    ...
    plt.grid()
    plt.savefig(f"losses_vs_epochs_{name}.png")
    plt.show()
...
class Baseline(nn.Module):
    ...
    def Metrics(self, pred_seg, gt_seg, pred_vendor=None, gt_vendor=None):
        ...
        #     metrics["v_precision"] = precision_score(gt_vendor, pred_vendor, average='weighted')
...
def infer_predictions(
    inference_folder, test_loader, model=None, validator=None, ae=None
):
    ...
    for patient in test_loader:
        ...
        for iter, batch in enumerate(patient):
            ...
            if ae is not None:
                ...
                reconstruction = (
                    torch.cat([reconstruction, batch["reconstruction"]], dim=0)
                    if len(reconstruction) > 0
                    else batch["reconstruction"]
                )
        if len(gt) != 0:
            gt = {
...
def evaluate_metrics(prediction, reference):
    results = {}
    for c, key in enumerate(["_LV", "_MYO", "_RV"], start=1):
        ...
        try:
            results["DSC" + key] = binary.dc(
                np.where(ref != 0, 1, 0), np.where(np.rint(pred) != 0, 1, 0)
            )
        except:
            results["DSC" + key] = 0
        try:
            results["HD" + key] = binary.hd(
                np.where(ref != 0, 1, 0), np.where(np.rint(pred) != 0, 1, 0)
            )
        except:
            results["HD" + key] = np.inf
    # print("evaluate_metrics", results)
    return results


def postprocess_predictions(
    inference_folder, patient_info, current_spacing, output_folder
):
    ...
    for prediction_file in os.listdir(inference_folder):
        ...
        prediction_file = np.load(
            os.path.join(inference_folder, prediction_file), allow_pickle=True
        ).item()
        prediction = prediction_file["prediction"]
        prediction = postprocess_image(
            prediction, patient_info[patient_id], phase, current_spacing
        )
        nib.save(
            nib.Nifti1Image(
                prediction,
                patient_info[patient_id]["affine"],
                patient_info[patient_id]["header"],
            ),
        ...
        if "reconstruction" in prediction_file:
            ...
            reconstruction = postprocess_image(
                reconstruction, patient_info[patient_id], phase, current_spacing
            )
            nib.save(
                nib.Nifti1Image(
                    reconstruction,
                ...
                os.path.join(
                    output_folder, "{}_{}_AE.nii.gz".format(patient_id, phase)
                ),
            )
    # print("postprocess_predictions", results)
    return results
...
def display_results(results):
    ...
    for axis in results:
        ...
        stats = {
            ...
            "LV_ED_DC": [df_ED["DSC_LV"].mean()],
            "LV_ED_HD": [df_ED["HD_LV"].mean()],
            "LV_ES_DC": [df_ES["DSC_LV"].mean()],
            "LV_ES_HD": [df_ES["HD_LV"].mean()],
            "LV_DC": [pd.concat([df_ED["DSC_LV"], df_ES["DSC_LV"]]).mean()],
            "LV_HD": [pd.concat([df_ED["HD_LV"], df_ES["HD_LV"]]).mean()],

            "MYO_ED_DC": [df_ED["DSC_MYO"].mean()],
            "MYO_ED_HD": [df_ED["HD_MYO"].mean()],
            "MYO_ES_DC": [df_ES["DSC_MYO"].mean()],
            "MYO_ES_HD": [df_ES["HD_MYO"].mean()],
            "MYO_DC": [pd.concat([df_ED["DSC_MYO"], df_ES["DSC_MYO"]]).mean()],
            "MYO_HD": [pd.concat([df_ED["HD_MYO"], df_ES["HD_MYO"]]).mean()],
        }
        table = pd.concat([table, pd.DataFrame.from_dict(stats)]).reset_index(drop=True)
    # display(table.set_index("axis"))
    print(table.set_index("axis"))
...
def optimize_segmentation(loader, model, ae=None, epoch=None, best_prediction=None):
    for batch in loader:
        ...
        if ae is None:
            ...
            batch["pseudo_gt"] = torch.mean(
                torch.stack(
                    [
                        epoch
                        / EPOCHS
                        * torch.tensor(best_prediction, requires_grad=False).to(device),
            ...
            batch["pseudo_gt"] = (
                torch.nn.functional.one_hot(
                    torch.argmax(batch["pseudo_gt"], dim=1), num_classes=4
                )
                .permute(0, 3, 1, 2)
                .float()
            )
...
def semisupervised_refinement(
    model, ae, epochs, train_loader, val_loader, ulab_loader, validator, checkpoint
):
    model.eval()
    with torch.no_grad():
        validator.domain_evaluation("val", model, val_loader, checkpoint)
    ...
    for epoch in epochs:
        ...
        epoch_end(
            {**validator.get_summary("val"), "#anom": num_anomalies},
            f"{epoch} - supervised training",
        )
        if num_anomalies == 0:
            break
        ...
        for phase in ["ED", "ES"]:
            for patient_id in anomalies[phase]:
                unlabelled_patient = torch.utils.data.DataLoader(
                    ACDCPatientPhase(
                        ulab_loader.root_dir,
                        patient_id,
                        phase,
                        transform=transform_downsample,
                    ),
                    batch_size=BATCH_SIZE,
                    shuffle=False,
                    num_workers=0,
                ...
                optimize_segmentation(
                    ...
                    ae,
                    epoch,
                    best_prediction=validator.get_best_prediction("ulab", patient_id)[
                        phase
                    ],
                )
        ...
        epoch_end(
            {**validator.get_summary("val"), "#anom": num_anomalies},
            f"{epoch} - semisupervised refinement",
        )
        if epoch % 10 == 0:
            checkpoint("{:03d}".format(epoch), model, validator)
        if num_anomalies == 0:
